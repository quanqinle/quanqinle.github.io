---
layout:       post
title:        "Linux | 同一个硬盘，将一个lvm分区的空间转移给另一个"
subtitle:     "CentOS根目录的空间将要用尽，但home还有很多空间，所以将home空间转移给根"
...
一台CentOS开发机报错“硬盘空间满了”，暂时又没有办法硬盘扩容。查看机器详情后发现它在使用lvm管理存储，根`/`和`/home`单独挂载在不同的逻辑卷，报错的是根`/`，但是`/home`还有足够的空间，所以可以把`/home`的空间转移给`/`。
...
备份home >> 卸载home >> 删除home >> 将home的空间添加到 `/` >> 重新分配home >> 格式化home >> 完成
